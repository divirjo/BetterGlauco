...
class ParametrosTabelas:
    CSS_PADRAO = {
        ...
        'tfoot': {'class': 'text-sm font-bold text-gray-900 px-6 py-4'},
    }
...
class ColunaSomaNumericaDecimal(tables.Column):
    ...
    def render_footer(bound_column, table):
        return '{:0.2f}'.format(
            sum(bound_column.accessor.resolve(linha) for linha in table.data)
        )
