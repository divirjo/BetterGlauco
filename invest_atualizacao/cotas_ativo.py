...
from investimento.models import Ativo, ExtratoOperacao, PosicaoDataBolsa, \
    PosicaoDataFundo, TipoOperacao
...
class CotaAtivo():
    ...
    def valorMaisRecente(self):
        ...
        if self._ativo.cota_bolsa:
            posicaoAtivo = PosicaoDataBolsa.objects \
                .filter(ativo=self._ativo.id) \
                .order_by('-data') \
                .first() 
            if posicaoAtivo:
                valorCota = posicaoAtivo.cota_valor
        else:
            posicaoAtivo = PosicaoDataFundo.objects \
                .filter(ativo_perfil_caixa__ativo=self._ativo.id) \
                .order_by('-data') \
                .first()
            if posicaoAtivo:
