...
from .forms import (
    FormEditarPosicaoDataFundo,
    FormNovaPosicaoDataFundo,
    FormPosicaoDataBolsa,
)
...
class PosicaoIndividualBolsa(LoginRequiredMixin, SingleTableMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        messages.warning(
            self.request,
            'Cadastro geral. \
                Alterações aqui afetarão todos os usuários do sistema',
        )
        return context
...
class PosicaoIndividualBolsaNova(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        query_ativos = Ativo.objects.filter(cota_bolsa=True).order_by(
            'ticket', 'nome'
        )
        ...
        context['id_perfil_selecionado'] = id_perfil
        context['nome_parametro'] = 'valor atualizado'
        context['url_retorno'] = 'invest_atualizacao:posicao_individual_bolsa'
    ...
    def form_valid(self, form, **kwargs):
        ...
        operacao.save()
        mensagem = 'Posição do ativo {} incluída com sucesso'.format(
            operacao.ativo.ticket
        )
...
class PosicaoIndividualBolsaEditar(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form, **kwargs):
        operacao = form.save(commit=False)
        operacao.save()
        mensagem = 'Posição do ativo {} atualizada com sucesso'.format(
            operacao.ativo.ticket
        )
        messages.success(self.request, mensagem)
        return redirect('invest_atualizacao:posicao_individual_bolsa')
...
class PosicaoIndividualFundo(LoginRequiredMixin, SingleTableMixin, FilterView):
    ...
    def get_filterset(self, *args, **kwargs):
        ...
        fs.filters[
            'ativo_perfil_caixa__corretora'
        ].field.queryset = InstituicaoFinanceira.objects.filter(
            perfil=id_perfil
        )
...
class PosicaoIndividualFundoNova(LoginRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['id_perfil_selecionado'] = id_perfil
        context['nome_parametro'] = 'valor atualizado'
        context['url_retorno'] = 'invest_atualizacao:posicao_individual_fundo'
    ...
    def form_valid(self, form, **kwargs):
        ...
        mensagem = 'Posição do ativo {} incluída com sucesso'.format(
            operacao.ativo_perfil_caixa.ativo.nome
        )
        messages.success(self.request, mensagem)
