...
class TabelaExtratoOperacoes(tables.Table):
    ...
    editar = tables.LinkColumn(
        'invest_operacao:operacao_individual_editar',
        text='atualizar',
...
class tabelaNotaCorretagem(tables.Table):
    id = tables.Column(verbose_name='id', footer='Total:')

    ativo_perfil_caixa_id = tables.Column(
        verbose_name='ID Ativo Perfil', footer='Total:'
    )
    ...
    custo_unitario_transacao = ColunaDinheiro(
        verbose_name='Custos transação (R$)'
    )
    ...
    editar = tables.LinkColumn(
        'invest_operacao:nota_corretagem',
        text='atualizar',
