...
class Imposto_bdr_usa(LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        valor_dividendos = Funcoes_auxiliares.converte_numero_str(
            self.request.GET.get('valor_dividendos')
        )
        ...
        self.tabela['Imposto de Renda EUA retido na fonte estimado (R$)'] = (
            imposto_retido
        )
...
class Valor_compra(LoginRequiredMixin, TemplateView):
    ...
    def monta_tabela_custos(self, bdr):
        valor_US_digitado = Funcoes_auxiliares.converte_numero_str(
            self.request.GET.get('valor_US')
        )
    ...
    def gera_linha_tabela(self, operacao, bdr):
        valor_BR_digitado = Funcoes_auxiliares.converte_numero_str(
            self.request.GET.get('valor_BDR')
        )

        self.calculo_bdr[operacao]['Ticket'] = bdr.ativo_selecionado.ticket
        self.calculo_bdr[operacao]['USA'] = (
            bdr.ativo_selecionado.ticket_original
        )
        self.calculo_bdr[operacao]['Desdobramento'] = int(
            bdr.ativo_selecionado.desdobramento
        )
        self.calculo_bdr[operacao]['Custo BDR'] = bdr.custos_bdr
        self.calculo_bdr[operacao]['Preço (U$)'] = bdr.ticket_original_valor_us

        if operacao == 'Venda':
            self.calculo_bdr[operacao]['Dólar (R$)'] = '{:.2f}'.format(
                bdr.dolar_compra
            )
            self.calculo_bdr[operacao]['Preço referência (R$)'] = (
                '{:.2f}'.format(bdr.preco_referencia_venda)
            )
        else:
            self.calculo_bdr[operacao]['Dólar (R$)'] = '{:.2f}'.format(
                bdr.dolar_venda
            ...
            self.calculo_bdr[operacao]['Preço referência (R$)'] = (
                '{:.2f}'.format(bdr.preco_referencia_compra)
            )

        self.calculo_bdr[operacao]['Preço atual (R$)'] = '{:.2f}'.format(
            valor_BR_digitado
        )
