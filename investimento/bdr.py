...
class BDR:
    ...
    def carregaAtivo(self, ticker_informado):
        ativos_localizados = Ativo.objects.filter(
            ticket__icontains=ticker_informado
        )

        if len(ativos_localizados) > 0:
            # Neste caso, qualquer ativo é aceito, pois os dados utilizados no
            # cálculo não tem relação com o perfil ativo
            return ativos_localizados[0]
        else:
            messages.error(
                self._request,
                'Ativo {} não cadastrado'.format(ticker_informado),
            )
            return Ativo()
    ...
    def get_cotacao_dolar(self):
        """
        Obtém o valor do dolar para venda e compra, consultando a API do Banco
        Central
        """
        data_atual = timezone.now().strftime('%m-%d-%Y')
        link_api_bacen = "https://olinda.bcb.gov.br/olinda/servico/PTAX/versao/v1/odata/CotacaoMoedaAberturaOuIntermediario(codigoMoeda=@codigoMoeda,dataCotacao=@dataCotacao)?@codigoMoeda='USD'&@dataCotacao='{}'&$format=json".format(
            data_atual
        )
    ...
    def get_cotacao_ticker_referencia(self):
        ...
        if self.ticket_original_valor_us == 0.0:
            messages.info(
                self._request,
                'Cotação internacional do ativo indisponível. '
                    'Exibindo dados de ontem',
            )
            data_anterior = (
                timezone.now() - timezone.timedelta(days=1)
            ).strftime('%Y-%m-%d')
            self.ticket_original_valor_us = self.consultar_api_cotacoes(
                data_anterior
            )

    def consultar_api_cotacoes(self, data_consulta):
        e_mail = self._CONSTANTE.E_MAIL_API_SCRAPERLINK()
        link_api = 'http://api.scraperlink.com/investpy/?email={}&type=historical_data&product=etfs&from_date={}&to_date={}&time_frame=Daily&country=united%20states&symbol={}'.format(
            e_mail,
            data_consulta,
            data_consulta,
            self.ativo_selecionado.ticket_original,
        )
    ...
    def get_preco_referencia(self):
        self.preco_referencia_compra = (
            self.dolar_venda
            * self.ticket_original_valor_us
            * self.custos_bdr
            / self.ativo_selecionado.desdobramento
        )
        self.preco_referencia_venda = (
            self.dolar_compra
            * self.ticket_original_valor_us
            * self.custos_bdr
            / self.ativo_selecionado.desdobramento
